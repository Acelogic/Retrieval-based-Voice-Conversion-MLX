        ...
        try:
            ...
            if weight_root.endswith(".pt"):
                ...
                if os.path.exists(alt_path):
                    ...
                    raise RuntimeError(
                        f"Cannot load {weight_root} (PyTorch format) in pure MLX mode. Please convert to .safetensors or .npz first using the migration tools."
                    )
            ...
            if weight_root.endswith(".npz") or weight_root.endswith(".safetensors"):
                ...
                if os.path.exists(config_path):
                    ...
                    if "enc_p.encoder.layers.0.attn.q_proj.weight" in weights:
                        ...
                        if w.shape[-1] == 256:  # v1
                            ...
                            self.cpt["config"][
                                4
                            ] = 256  # filter_channels? No v1 filter channels usually 768, but hidden size 256?
                ...
                raise ValueError(
                    "Unsupported model file format. Use .safetensors or .npz"
                )
    ...
    def voice_conversion(
        ...
        if self.index is not None and index_rate > 0:
            ...
            if search_query.shape[0] > 0:
                ...
                # Blend
                npy[skip_offset:] = (
                    new_feats * index_rate + (1 - index_rate) * npy[skip_offset:]
                )
        ...
        if scaled_window != self.model_window:
            out_audio_np = librosa.resample(
                out_audio_np, orig_sr=scaled_window, target_sr=self.model_window
            )
