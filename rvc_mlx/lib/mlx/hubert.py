...
class HubertAttention(nn.Module):
    ...
    def __call__(
        ...
        q = q.reshape(
            B, L, self.num_attention_heads, self.attention_head_size
        ).transpose(0, 2, 1, 3)
        k = k.reshape(
            B, L, self.num_attention_heads, self.attention_head_size
        ).transpose(0, 2, 1, 3)
...
class HubertGroupNormConvLayer(nn.Module):
    def __init__(
        self, config, in_channels, out_channels, kernel_size, stride, bias=False
    ):
        super().__init__()
        self.conv = nn.Conv1d(
            in_channels,
    ...
    def __init__(self, config: HubertConfig):
        ...
        for k, s in zip(kernels, strides):
            self.conv_layers.append(
                HubertNoLayerNormConvLayer(config, out_d, out_d, k, s)
            )
