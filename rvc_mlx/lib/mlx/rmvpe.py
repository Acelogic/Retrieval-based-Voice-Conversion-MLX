...
class Encoder(nn.Module):
    def __init__(
        ...
        for i in range(n_encoders):
            self.layers.append(
                ResEncoderBlock(curr_in, curr_out, kernel_size, n_blocks, momentum)
            )
            curr_in = curr_out
            curr_out *= 2
...
class Decoder(nn.Module):
    def __init__(self, in_channels, n_decoders, stride, n_blocks, momentum=0.01):
        ...
        for _ in range(n_decoders):
            ...
            self.layers.append(
                ResDecoderBlock(curr_in, out_channels, stride, n_blocks, momentum)
            )
...
class BiGRU(nn.Module):
    def __init__(self, input_features, hidden_features, num_layers):
        ...
        for i in range(num_layers):
            ...
            setattr(
                self.forward_grus, str(i), nn.GRU(in_size, hidden_features, bias=True)
            )
            setattr(
                self.backward_grus, str(i), nn.GRU(in_size, hidden_features, bias=True)
            )
...
class RMVPE0Predictor:
        ...
        if weights_path is None:
            ...
            if not os.path.exists(weights_path):
                ...
                rel_path = os.path.join(
                    ...
                    "../../../models/predictors/rmvpe_mlx.npz",
                )
                if os.path.exists(rel_path):
                    ...
                    weights_path = os.path.join(
                        "rvc", "models", "predictors", "rmvpe_mlx.npz"
                    )
    ...
    def _create_mel_filterbank(self, n_fft, n_mels, sr, fmin, fmax):
        ...
        for i in range(n_mels):
            ...
            filterbank[i, left_mask] = (freq_bins[left_mask] - left) / (
                center - left + 1e-10
            )

            # Right slope
            right_mask = (freq_bins >= center) & (freq_bins <= right)
            filterbank[i, right_mask] = (right - freq_bins[right_mask]) / (
                right - center + 1e-10
            )
