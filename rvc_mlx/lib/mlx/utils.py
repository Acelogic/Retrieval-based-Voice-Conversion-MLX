...
def load_embedding(embedder_model: str = None, embedder_model_custom: str = None) -> nn.Module:
    """
    ...
    if not model_path or not os.path.exists(model_path):
        ...
        candidates = [
            "hubert_base.pt", "models/hubert_base.pt", "rvc_mlx/models/hubert_base.pt",
            "hubert_base.safetensors", "models/hubert_base.safetensors"
        ]
        for c in candidates:
            if os.path.exists(c):
                model_path = c
    ...
    if model_path and model_path.endswith(".pt"):
        print(f"Warning: {model_path} is a PyTorch checkpoint. Please convert it to MLX format using tools/convert_hubert.py first.")
        ...
        if os.path.exists(safe_path):
            ...
            raise RuntimeError("Cannot load .pt file in pure MLX mode. Please convert weights.")
