...
class SineGenerator(nn.Module):
    ...
    def _generate_sine_wave(self, f0, upsampling_factor):
        ...
        harmonic_scale = mx.arange(1, self.waveform_dim + 1, dtype=f0.dtype).reshape(
            1, 1, -1
        )
        phase_increments = phase_increments * harmonic_scale

        # Random phase
        random_phase = mx.random.uniform(shape=(1, 1, self.waveform_dim)).astype(
            f0.dtype
        )
        ...
        idxRest = mx.random.uniform(shape=(1, 1, self.waveform_dim - 1)).astype(
            f0.dtype
        )
    ...
    def __call__(self, f0, upsampling_factor):
        ...
        noise_amp = voiced_mask * self.noise_std + (1 - voiced_mask) * (
            self.sine_amp / 3
        )
    ...
    def __init__(
        ...
        for i, (u, k) in enumerate(zip(upsample_rates, upsample_kernel_sizes)):
            ...
            self.noise_convs.append(
                nn.Conv1d(1, channels[i], kernel, stride=stride, padding=padding)
            )
        ...
        self.conv_post = nn.Conv1d(
            channels[-1], 1, 7, 1, padding=3
        )  # remove bias? orig said bias=False
        ...
        for i, l in enumerate(self.ups):
            setattr(self, f"up_{i}", l)
        for i, l in enumerate(self.noise_convs):
            setattr(self, f"noise_conv_{i}", l)
        for i, l in enumerate(self.resblocks):
            setattr(self, f"resblock_{i}", l)
