...
def convert_weights(pth_path, output_path):
    ...
    if pth_path.endswith(".safetensors"):
        try:
            ...
            print("Error: safetensors not installed. Please install it with 'pip install safetensors'.")
            return
        cpt = {"weight": state_dict, "config": {}} # Mock cpt for safetensors which usually just has weights
    else:
