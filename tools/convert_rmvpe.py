...
def convert():
    ...
    for key, val in ckpt.items():
        ...
        if "fc.0.gru." in key:
            ...
            new_key = new_key.replace('fc.0.gru.weight_ih_l0_reverse', 'bigru.backward_grus.0.Wx')
            new_key = new_key.replace('fc.0.gru.weight_hh_l0_reverse', 'bigru.backward_grus.0.Wh')
            new_key = new_key.replace('fc.0.gru.bias_ih_l0_reverse', 'bigru.backward_grus.0.b')
            if 'bias_hh_l0_reverse' in key:
                new_key = 'bigru.backward_grus.0.bhn'
                val = val[-H:]  # Extract last H elements
                
            # Forward
            new_key = new_key.replace('fc.0.gru.weight_ih_l0', 'bigru.forward_grus.0.Wx')
            new_key = new_key.replace('fc.0.gru.weight_hh_l0', 'bigru.forward_grus.0.Wh')
