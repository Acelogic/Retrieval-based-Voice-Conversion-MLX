...
def run_inference(backend):
    ...
    time_match = re.search(
        r"Conversion completed at .* in ([\d\.]+) seconds", result.stdout
    )
...
def main():
    ...
    print("\n" + "=" * 70)
    print(
        f"{'Backend':<10} | {'Median Time':<12} | {'Median Mem':<12} | {'Max Mem':<10}"
    )
    print("-" * 70)
    ...
    for backend in ["torch", "mlx"]:
        ...
        if times:
            ...
            m_max = np.max(mems)
            print(
                f"{backend:<10} | {t_med:<12.3f} | {m_med:<12.1f} MB | {m_max:<10.1f} MB"
            )
        else:
