...
def parse_arguments():
    ...
    infer_parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to MLX .npz model",
    )
    infer_parser.add_argument(
        "--input_path", type=str, required=True, help="Input audio path"
    )
    infer_parser.add_argument(
        "--output_path", type=str, required=True, help="Output audio path"
    )

    infer_parser.add_argument(
        "--pitch", type=int, default=0, help="Pitch shift semitones"
    )
    infer_parser.add_argument(
        "--f0_method",
        type=str,
        default="rmvpe",
        choices=["rmvpe"],
        help="F0 method (only rmvpe supported in MLX currently)",
    )
    infer_parser.add_argument(
        "--index_path", type=str, default="", help="Path to .index file"
    )
    infer_parser.add_argument(
        "--index_rate", type=float, default=0.75, help="Index rate"
    )
    infer_parser.add_argument(
        "--volume_envelope", type=float, default=1.0, help="Volume envelope scaling"
    )
    infer_parser.add_argument(
        "--protect", type=float, default=0.5, help="Protect voiceless consonants"
    )
    ...
    infer_parser.add_argument(
        "--f0_autotune", type=lambda x: bool(strtobool(x)), default=False
    )
    ...
    for arg in [
        ...
        infer_parser.add_argument(
            arg, action="store_true", help="Ignored in MLX version"
        )
    ...
    for arg in [
        ...
        infer_parser.add_argument(
            arg, type=str, help="Ignored in MLX version"
        )  # Type str to catch whatever
