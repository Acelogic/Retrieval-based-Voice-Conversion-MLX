...
def convert():
    ...
    for key, val in ckpt.items():
        ...
        if "fc.0.gru." in key:
            ...
            new_key = new_key.replace('fc.0.gru.weight_ih_l0_reverse', 'fc.layers.0.0.backward_grus.0.Wx')
            new_key = new_key.replace('fc.0.gru.weight_hh_l0_reverse', 'fc.layers.0.0.backward_grus.0.Wh')
            new_key = new_key.replace('fc.0.gru.bias_ih_l0_reverse', 'fc.layers.0.0.backward_grus.0.b')
            if 'bias_hh_l0_reverse' in key:
                new_key = 'fc.layers.0.0.backward_grus.0.bhn'
                val = val[-H:]  # Extract last H elements
                
            # Forward
            new_key = new_key.replace('fc.0.gru.weight_ih_l0', 'fc.layers.0.0.forward_grus.0.Wx')
            new_key = new_key.replace('fc.0.gru.weight_hh_l0', 'fc.layers.0.0.forward_grus.0.Wh')
            new_key = new_key.replace('fc.0.gru.bias_ih_l0', 'fc.layers.0.0.forward_grus.0.b')
            if 'bias_hh_l0' in key and 'reverse' not in key:
                new_key = 'fc.layers.0.0.forward_grus.0.bhn'
                val = val[-H:]  # Extract last H elements
            
        # 5. FC Linear: fc.1 -> fc.layers.0.1
        if "fc.1." in new_key:
            new_key = new_key.replace('fc.1.', 'fc.layers.0.1.')
        ...
        if "conv1.0." in key and "weight" in key and val.ndim == 4:
            ...
            val = np.flip(val, axis=(1, 2)).copy()
        # Regular Conv2d (exclude conv1.0 which is ConvTranspose2d)
        elif ("conv" in key or key.startswith("cnn")) and "weight" in key and val.ndim == 4:
    ...
    save_path = "rvc/models/predictors/rmvpe_mlx.npz"
